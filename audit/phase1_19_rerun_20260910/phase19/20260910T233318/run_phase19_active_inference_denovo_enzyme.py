...
def place_free_bundle(tz, rng, flow_dirs=None):
    ...
    def _aimed_rod(ca, v, n, target, htype="CB"):
        ...
        if htype == "TRP":
            rod = trp_optimal_anchor_roll(rod, v, target)
...
def trp_optimal_anchor_roll(rod, axis, target, anchor_index=1):
    """Rigid roll attaining the radial lower bound, when attainable by roll.

    For a target beyond maximum CA reach R, the optimal NE1 is R*d_hat.
    The CB-centered spherical band's upper projection zmax therefore needs
    CB_hat.d_hat = (|CA-CB| + zmax)/R. A roll about the rod axis makes this
    dot product A*cos(theta)+B*sin(theta)+C, solved analytically below.
    Keep the old orientation if the target is inside R or no roll realizes
    this condition. This optimizes NE1 reach only; existing collision and
    constellation gates still apply, and chi fitting is still required.
    """
    residue = rod[anchor_index]
    ca = np.asarray(residue["CA"], float)
    d = np.asarray(target, float) - ca
    length = np.linalg.norm(d)
    diagnostic = trp_target_reachability(residue, target)
    reach = diagnostic["maximum_CA_reach_A"]
    if length <= reach:
        return rod
    d = d / length
    axis = np.asarray(axis, float)
    axis = axis / np.linalg.norm(axis)
    cb = place_cb(residue) - ca
    bond = np.linalg.norm(cb)
    u = cb / bond
    desired = (bond + diagnostic["projection_limits_A"][1]) / reach
    parallel = axis * (u @ axis)
    a = float((u - parallel) @ d)
    b = float(np.cross(axis, u) @ d)
    c = float(parallel @ d)
    amplitude = math.hypot(a, b)
    if amplitude < 1e-12 or abs(desired - c) > amplitude:
        return rod
    phase = math.atan2(b, a)
    delta = math.acos(np.clip((desired - c) / amplitude, -1., 1.))
    angles = [(phase + sign * delta + math.pi) % (2 * math.pi) - math.pi
              for sign in (-1., 1.)]
    rotation = _rot_about_axis(axis, min(angles, key=abs))
    return [{key: ca + rotation @ (value - ca) for key, value in r.items()}
            for r in rod]
