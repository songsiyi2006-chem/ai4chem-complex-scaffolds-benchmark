...
def build_markdown(records: List[Dict[str, Any]], meta: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append(f"# Complex Molecule Benchmark: {len(records)} Registry Entries")
    lines.append("")
    ...
    header = ("| # | Molecule | MW (Da) | cLogP | TPSA (Å²) | Fsp³ | RotB | Stereo a/u | "
              "MaxRing | Conf accepted/embedded | FF | E_min | ΔE_ens | IMHB | d_min (Å) | Risk |")
    lines.append(header)
    lines.append("|" + "---|" * 16)
    for r in records:
        ...
        if r["status"] == "failed_parse":
            ...
            row = (f"| {r['id']} | {r['name']} | {_fmt(p.get('mw_full_input'))} | "
                   ...
                   f"{g.get('max_ring', '-')} | "
                   f"{str(c.get('n_accepted', '-')) + '/' + str(c.get('n_embedded', '-')) if c.get('status') == 'ok' else 'fail'} | "
    ...
    for r in records:
        ...
        if r.get("conformers", {}).get("status") == "ok":
            ...
            lines.append(f"- **Ensemble:** {c.get('n_accepted', '-')} accepted / {c['n_embedded']} embedded conformers with {c['ff']} "
