...
metadata = {
    'protocolName': 'Phase-9 SDL Round-1 batch — aziridine ring expansion (CPA)',
    'author': 'AI4Chem Phase-9 Self-Driving Lab Compiler',
    'description': '5 q-EI conditions + HPLC serial dilution; guardrail-verified',
    'apiLevel': '2.15',
}
...
def run(ctx: protocol_api.ProtocolContext) -> None:
    ...
    v_cat_ul = 4.26  # catalyst dose at 25 mM
    v_mix_ul = 135.74          # blend makeup volume
    phi_tol  = 0.1671           # toluene volume fraction
    ...
    # (3) reaction hold  t = 4.89 h  (robot idles; temp module holds)
    ctx.delay(minutes=293.5)
    ...
    v_cat_ul = 4.26  # catalyst dose at 25 mM
    v_mix_ul = 135.74          # blend makeup volume
    phi_tol  = 0.8962           # toluene volume fraction
    v_tol = v_mix_ul * phi_tol
    v_dcm = v_mix_ul - v_tol
    ...
    # (3) reaction hold  t = 1.51 h  (robot idles; temp module holds)
    ctx.delay(minutes=90.3)
    ...
    # ---- experiment 3: R1-3 -> plate C1 ----
    T_SET = 61.3
    assert 4.0 <= T_SET <= 95.0, 'temperature outside Temperature Module range'
    temp_mod.set_temperature(T_SET)
    temp_mod.await_temperature(T_SET)
    v_cat_ul = 4.14  # catalyst dose at 25 mM
    v_mix_ul = 135.86          # blend makeup volume
    phi_tol  = 0.9891           # toluene volume fraction
    v_tol = v_mix_ul * phi_tol
    v_dcm = v_mix_ul - v_tol
    ...
    # (3) reaction hold  t = 3.57 h  (robot idles; temp module holds)
    ctx.delay(minutes=214.1)
    ...
    # ---- experiment 4: R1-4 -> plate D1 ----
    T_SET = 40.9
    assert 4.0 <= T_SET <= 95.0, 'temperature outside Temperature Module range'
    temp_mod.set_temperature(T_SET)
    temp_mod.await_temperature(T_SET)
    v_cat_ul = 4.11  # catalyst dose at 25 mM
    v_mix_ul = 135.89          # blend makeup volume
    phi_tol  = 0.9941           # toluene volume fraction
    v_tol = v_mix_ul * phi_tol
    v_dcm = v_mix_ul - v_tol
    ...
    # (3) reaction hold  t = 21.52 h  (robot idles; temp module holds)
    ctx.delay(minutes=1291.2)
    ...
    # ---- experiment 5: R1-5 -> plate E1 ----
    T_SET = 35.8
    assert 4.0 <= T_SET <= 95.0, 'temperature outside Temperature Module range'
    temp_mod.set_temperature(T_SET)
    temp_mod.await_temperature(T_SET)
    v_cat_ul = 4.22  # catalyst dose at 25 mM
    v_mix_ul = 135.78          # blend makeup volume
    phi_tol  = 0.9602           # toluene volume fraction
    v_tol = v_mix_ul * phi_tol
    v_dcm = v_mix_ul - v_tol
    ...
    if v_tol >= 30.0:
        p300.pick_up_tip()
        _dose(p300, solv.wells_by_name()[SOLVENT_WELLS['toluene']],
              plate.wells_by_name()[CONDITIONS[4]['well']], v_tol, FLOWS['toluene'][0], FLOWS['toluene'][1], prewet=2)
        p300.drop_tip()
    elif v_tol >= 5.0:
        p20.pick_up_tip()
        _dose(p20, solv.wells_by_name()[SOLVENT_WELLS['toluene']],
    ...
    if v_dcm >= 30.0:
        p300.pick_up_tip()
        _dose(p300, solv.wells_by_name()[SOLVENT_WELLS['dcm']],
              plate.wells_by_name()[CONDITIONS[4]['well']], v_dcm, FLOWS['dcm'][0], FLOWS['dcm'][1], prewet=2)
        p300.drop_tip()
    elif v_dcm >= 5.0:
        p20.pick_up_tip()
        _dose(p20, solv.wells_by_name()[SOLVENT_WELLS['dcm']],
    ...
    p20.pick_up_tip()
    _dose(p20, reag.wells_by_name()[REAGENT_WELLS['substrate']],
          plate.wells_by_name()[CONDITIONS[4]['well']], 40.0,
          FLOWS['toluene'][0], FLOWS['toluene'][1], prewet=1)
    # (2) catalyst stock (viscous-free toluene class, volumetric accuracy)
    _dose(p20, reag.wells_by_name()[REAGENT_WELLS['catalyst']],
          plate.wells_by_name()[CONDITIONS[4]['well']], v_cat_ul,
          FLOWS['toluene'][0] * 0.8, FLOWS['toluene'][1] * 0.8, prewet=2)
    p20.mix(8, 18, plate.wells_by_name()[CONDITIONS[4]['well']])
    p20.drop_tip()

    # (3) reaction hold  t = 12.17 h  (robot idles; temp module holds)
    ctx.delay(minutes=729.9)

    # (4) quench: MeOH + internal standard (viscous DMSO IS class)
    p20.pick_up_tip()
    q_well = plate.wells_by_name()[CONDITIONS[4]['well']]
    _dose(p20, solv.wells_by_name()[SOLVENT_WELLS['meoh']], q_well, 15.0,
          FLOWS['meoh'][0], FLOWS['meoh'][1], prewet=1)
    _dose(p20, reag.wells_by_name()[REAGENT_WELLS['is_stock']], q_well, 5.0,
          FLOWS['dmso'][0], FLOWS['dmso'][1], prewet=3)
    ...
    # (5) HPLC serial dilution: 5 uL reaction + 195 uL diluent (1:40),
    #     then 10 uL + 190 uL diluent (1:20) -> 1:800 analytical dilution
    v1 = vials.wells_by_name()['A3']
    v2 = vials.wells_by_name()['B3']
    p300.pick_up_tip()
    _dose(p300, solv.wells_by_name()[SOLVENT_WELLS['hplc_diluent']], v1, 195.0,
          FLOWS['hplc_diluent'][0], FLOWS['hplc_diluent'][1])
    p300.drop_tip()
