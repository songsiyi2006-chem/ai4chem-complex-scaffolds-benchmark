...
def atomic_scf(z_nuc, config, relativistic=True, n_pts=640, a_mass=None,
               max_iter=26, tol=3e-6, label="atom", c=C_LIGHT):
    ...
    e_old = None
    for it in range(max_iter):
        u_e = hartree_potential(rho, x, w)
        v_x = xalpha_vx(rho)
        v_pot = v_nuc + u_e + v_x
        orbs = solve_once(v_pot, it)
        ...
        for (n, kap), occ in occ_map.items():
            ...
            if (n, kap) not in orbs:
                missing.append((n, kap))
                continue
            e, P, Q = orbs[(n, kap)]
            rho_new += occ * orbital_density(P, Q, x, w)
            e_sum += occ * e
        if missing:
            raise RuntimeError(f"{label}: orbitals missing {missing} (it {it}); "
                               f"have {sorted(orbs)}")
    ...
    # final solve on the converged potential
    u_e = hartree_potential(rho, x, w)
    v_x = xalpha_vx(rho)
    v_pot = v_nuc + u_e + v_x
...
def breit_pair_tabulation(kappa, kernel="breit", chunk=20000):
    ...
    psi12 = (np.kron(v_up, v_dn) - np.kron(v_dn, v_up)) / math.sqrt(2.0)
    a1a2 = (np.kron(ax, ax) + np.kron(ay, ay) + np.kron(az, az))
    K9 = np.stack([np.kron(mi, mj) for mi in (ax, ay, az)
                   for mj in (ax, ay, az)])            # (9, 16, 16)
    I16 = np.eye(16, dtype=complex)
    flat = npts * npts
    for iu, u in enumerate(u_grid):
        r12v = n1[:, None, :] - u * n1[None, :, :]
        r12m = np.linalg.norm(r12v, axis=-1)
        r12 = r12m.reshape(-1)
        r12h = (r12v / r12m[..., None]).reshape(-1, 3)
        w12 = (wg[:, None] * wg[None, :]).reshape(-1)
        Amat = np.zeros((16, 16), dtype=complex)
        for p0 in range(0, flat, chunk):
            sl = slice(p0, min(p0 + chunk, flat))
            nc = len(r12[sl])
            if kernel == "breit":
                T = np.zeros((nc, 16, 16), dtype=complex)
                for q in range(9):
                    ii, jj = q // 3, q % 3
                    T += (r12h[sl][:, ii] * r12h[sl][:, jj])[:, None, None] \
                        * K9[q][None, :, :]
                K = -(a1a2[None, :, :] + T) / (2.0 * r12[sl])[:, None, None]
            elif kernel == "gaunt":
                K = np.broadcast_to(-a1a2[None] / (2.0 * r12[sl])[:, None, None],
                                    (nc, 16, 16))
            else:
                K = np.broadcast_to(I16[None] / r12[sl][:, None, None],
                                    (nc, 16, 16))
            bil = (psi12[sl].conj()[:, :, None] * K
                   * psi12[sl][:, None, :])
            Amat += np.einsum("p,pab->ab", w12[sl], bil)
        A[:, :, :, :, iu] = Amat.reshape(4, 4, 4, 4)
    return {"u": u_grid, "A_dir": A, "kappa": kappa, "kernel": kernel}


def breit_pair_energy(tab, P, Q, x, w, atol=1e-9):
    ...
    qq = Q if Q is not None and len(Q) == len(P) else np.zeros_like(P)
    fr = (P, P, qq, qq)
    u_r = np.sqrt(np.minimum.outer(x, x) / np.maximum.outer(x, x))
    u_c = np.clip(u_r, u[0], u[-1])
    r_pair = 1.0 / np.maximum.outer(x, x)
    e = 0.0 + 0.0j
        ...
        for be in range(4):
            for ga in range(4):
                for de in range(4):
                    ...
                    g1 = fr[al] * fr[ga] * w
                    g2 = fr[be] * fr[de] * w
                    Ar = np.interp(u_c.ravel(), u, tbl.real)
                    Ai = np.interp(u_c.ravel(), u, tbl.imag)
                    Ad = (Ar + 1j * Ai).reshape(u_c.shape)
                    e += float(np.sum(g1[:, None] * g2[None, :] * r_pair * Ad))
    return e
...
def eda_qtaim_stage(atom_am, atom_eu, atom_am_nr, atom_eu_nr, atom_n,
                    results):
    """Two-state EDA + QTAIM on the N-M-N axis for Am(III) vs Eu(III)."""
    ...
    for ion, atom in (("Am", atom_am), ("Eu", atom_eu)):
        ...
        d2x = (rho2[i_bcp + 1, 0] - 2 * rho2[i_bcp, 0]
               + rho2[i_bcp - 1, 0]) / dx ** 2
        d2z = (rho2[i_bcp, 2] - 2 * rho2[i_bcp, 1] + rho2[i_bcp, 0]) / dz ** 2
        lap_bcp = d2x + 2.0 * d2z
        rho_bcp = float(rho2[i_bcp, 1])
        # local kinetic model G = TF + (1/9) vW; H = 1/4 lap - G (a.u.)
        cf = 0.3 * (3 * math.pi ** 2) ** (2.0 / 3.0)
        grad_x = (rho2[i_bcp + 1, 1] - rho2[i_bcp - 1, 1]) / (2 * dx)
        grad_z = (rho2[i_bcp, 2] - rho2[i_bcp, 0]) / (2 * dz)
...
def fig2_multiplets(res):
    ...
    for ax, ion in zip(axes, ("Am", "Eu")):
        ...
        ax.text(0.03, 0.97,
                f"$\zeta$({sh}) from Dirac j-splitting = {zeta:.0f} cm$^{{-1}}$  "
                f"$\Delta E_{{SO}}$(J=1) = {ladder[1]:.0f} cm$^{{-1}}$  "
...
def fig3_qtaim(res):
    ...
    for k, ion in enumerate(("Am", "Eu")):
        ...
        lv = np.array([-1, -0.5, -0.2, -0.08, -0.03, 0.03, 0.08, 0.2, 0.5,
                       1, 2, 5])
        im = ax.contourf(xs, zs, lap, levels=40, cmap="RdBu_r",
                         norm=SymLogNorm(linthresh=0.05, vmin=-8, vmax=8))
        cs = ax.contour(xs, zs, lap, levels=lv, colors="k", linewidths=0.5)
        ax.plot([-pl["x_bcp"], pl["x_bcp"]], [0, 0], "w-", lw=2.4, alpha=0.9)
        ax.plot(pl["x_bcp"], 0, "o", ms=9, mfc="none", mec="k", mew=1.6)
        ax.annotate("BCP", (pl["x_bcp"], 0), textcoords="offset points",
                    xytext=(6, 10), fontsize=10)
        ax.set_xlabel("x along N-M-N (a$_0$)")
        ax.set_ylabel("z ($a_0$)")
        q = bond["qtaim"][ion]
        ax.set_title(f"{'Am' if ion == 'Am' else 'Eu'}$^{{3+}}$: "
                     f"$\\nabla^2\\rho$ through N-M-N   "
    ...
    axc.bar(xg + 0.18, ec, width=0.36, color="#c0392b",
            label="$\\Delta E_{cov}$ per [M(BTP)$_3$]$^{3+}$")
    ...
    axc.set_title(f"(c)  EDA covalent stabilization: selectivity "
    ...
    axd.set_title("(d)  QTAIM & overlap descriptors (bars grouped by ion)",
                  loc="left", fontsize=11)
    axd.legend(frameon=False, fontsize=9)
    axd.grid(alpha=0.25, lw=0.5, axis="y")
    fig.suptitle("Phase 17 - relativistic QTAIM & EDA: why Am$^{3+}$ coheres "
                 "with nitrogen donors and Eu$^{3+}$ does not",
...
def fig4_validation(res, kb_demo):
    ...
    ax.set_title("(b) Breit-machinery certification: the 6D angular-spinor quadrature\n"
                 "reproduces the exact Coulomb pair integral 5Z/8", loc="left", fontsize=10)
    ax.grid(alpha=0.3, lw=0.5)
    ...
    fig.suptitle("Phase 17 - foundation validation: exact solutions, "
                 "variational stability, Breit scaling & the covalency "
                 "mechanism", fontsize=13, fontweight="bold", y=0.99)
...
def gaussian_kb_demo(z=92.0, n_basis=10, c=C_LIGHT):
    """Matrix-Dirac H-like ion in s-Gaussian bases: kinetic balance vs raw."""
    x, y, h, w = log_grid(1500, 1e-7, 40.0)
    v = -z / x

    def basis_mats(balanced):
        # large: s-type gaussians chi_i = exp(-a r^2); small: (sigma.p)chi/2c
        ex = np.logspace(-3, 5, n_basis) * z
        L = np.exp(-np.outer(x ** 2, ex))
        # normalize large components
        nrm = np.sqrt(np.sum(L ** 2 * w[:, None], axis=0))
        L /= nrm[None, :]
        if balanced:
            # (sigma.p) e^{-a r^2} spinor ~ 2 a r e^{-a r^2} (radial part)
            S = 2.0 * (x[:, None] * np.exp(-np.outer(x ** 2, ex))) * ex[None, :]
        else:
            S = np.exp(-np.outer(x ** 2, ex * 0.5))
        sn = np.sqrt(np.sum(S ** 2 * w[:, None], axis=0) + 1e-300)
        S /= sn[None, :]
        nb = n_basis
        dim = 2 * nb
        H = np.zeros((dim, dim))
        Ov = np.eye(dim)
        def quad(f1, f2):
            return float(np.sum(f1 * f2 * w))
        for i in range(nb):
            for j in range(nb):
                Ov[i, j] = quad(L[:, i], L[:, j])
                Ov[i, j] += quad(S[:, i], S[:, j])
                Ov[nb + i, nb + j] = Ov[i, j]
                H[i, j] = quad(L[:, i], v * L[:, j])
                H[i, j] += quad(S[:, i], (v - 2 * c * c) * S[:, j])
                dL = np.gradient(L[:, j], x)
                dS = np.gradient(S[:, j], x)
                H[i, nb + j] = -c * quad(L[:, i], dS + S[:, j] / x)
                H[nb + i, j] = c * quad(S[:, i], dL + L[:, j] / x)
        Ov = 0.5 * (Ov + Ov.T)
        for jit in (1e-12, 1e-10, 1e-8, 1e-6):
            try:
                return sla.eigh(H, Ov + jit * np.eye(dim))[0]
            except Exception:
                continue
    ...
    log(f"  17A KB demo: balanced 1s = {ev_b[0]:.5f} Eh (exact {e_exact:.5f}); "
        f"unbalanced lowest = {ev_u[0]:.5f}, "
        f"{np.sum(ev_u < ev_b[0] - 0.5)} spurious states below")
    return [("kinetically balanced", ev_b[:8], "#16a085"),
...
def main():
    ...
    if args.stage in ("all", "breit"):
        ...
        br = {}
        tab_c = breit_pair_tabulation(-1, kernel="coulomb")
        zs = [1, 20, 50, 80]
        e1s = []
        xx = np.linspace(1e-5, 40, 4000)
        for Z in zs:
        ...
        tab_b = breit_pair_tabulation(-1, kernel="breit")
        tab_g = breit_pair_tabulation(-1, kernel="gaunt")
        hb_z, hb_cm = [], []
        for Z in (20, 50, 80, 92):
            xx = np.linspace(1e-6, 40, 3000)
            ww = np.gradient(xx)
